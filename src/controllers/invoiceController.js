...
//   },
  ...
  instructer_name: booking?.instructer?.name?booking?.instructer?.name:null,
  vehicle_info: booking?.instructer?.vehicle_model?booking?.instructer?.vehicle_model:null,
      };
...
function generateBookingsInvoiceContent(doc, data) {
  ...
  doc.font('Helvetica')
   ...
   //   .text(data.invoice_id || 'N/A', rightX + 70, 110)
     .text(data.date ?  new Date(data.date).toLocaleDateString() + ' ' + data?.selectedTime : 'N/A', rightX + 70, 120);
...
//   }
  ...
  doc.fontSize(10)
     .font('Helvetica')
     .fillColor(black)
     .text(`Instructer Name: ${data.instructer_name || 'N/A'}`, 50, currentY + 15)
